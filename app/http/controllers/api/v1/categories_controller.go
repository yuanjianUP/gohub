...
func (ctrl *CategoriesController) Index(c *gin.Context) {
	...
	if ok := requests.Validate(&request, c, requests.Pagination); !ok {
		return
	}
	data, paging := category.Paginate(c, 10)
...
func (ctrl *CategoriesController) Update(c *gin.Context) {
	...
	//表单验证
	request := requests.CategoryRequest{}
	if ok := requests.Validate(&request, c, requests.CategorySave); !ok {
		return
	}
